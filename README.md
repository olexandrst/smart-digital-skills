...
**AI Knowledge Hub** — внутрішня корпоративна AI-платформа Metinvest Digital:
єдина точка входу до AI-знань компанії (навички, промпти, інструкції, кейси,
агенти, MCP-сервери й корисні посилання), централізований доступ до моделей ШІ
у контурі Azure AI Foundry, рольова модель (RBAC), групи та облік токенів.
...
- 🗂️ **AI Knowledge Hub** у фірмовому стилі Metinvest Digital: крім навичок —
  **промпти** (копіювання в один клік), **інструкції** та **кейси**
  (Markdown-документи), **агенти**, **MCP-сервери** та **корисні посилання**
  (зовнішні сервіси й внутрішні ресурси); фільтри за видом і категорією, пошук,
  «Обране» (зірочка) для будь-якого елемента
- 🔌 **MCP-сервери** — картка містить endpoint (копіюється одним кліком у конфіг
  клієнта, а не відкривається в браузері), перелік доступних інструментів та
  інструкцію підключення; внутрішні сервери позначені окремо від публічних
- 🧭 **Розділи каталогу** — друга вісь навігації («Business Box»): дзеркалить
  розділи корпоративного хабу. Розділ із посиланням веде до зовнішнього
  порталу (гібридний режим), розділ без посилання показує власні матеріали
- ♻️ **Життєвий цикл матеріалу**: `Чернетка → Опублікований → Потребує
...
- 📈 **Аналітика хабу** («Навички → Аналітика»): наповнення за видами й
  розділами, актуальність контенту, успішність пошуку, ТОП-10 матеріалів
...
```bash
python -m scripts.init_db    # версійні міграції (Alembic) + довідники
python -m scripts.seed       # наповнює демо-даними (ролі, користувачі, скіл)
```

> **Оновлення наявної бази.** Схема ведеться версійними міграціями:
> після `git pull` виконайте `alembic upgrade head` (або `python -m scripts.init_db`,
> який робить це сам і додатково доганяє базу, створену до переходу на
> міграції). Видаляти `instance/profihub.db` не потрібно.
> Довідники й словники синхронізуються при старті автоматично — це дані, а не
> схема. Запасний варіант зі старою поведінкою: `AUTO_MIGRATE=1` у `.env`.
...
| GET | `/catalog/sections` | Розділи каталогу з кількістю матеріалів | авторизовані |
...
| POST | `/catalog/search-log/{id}/opened` | Позначка «запит завершився відкриттям» | авторизовані |
...
| GET | `/catalog/notifications` | Сповіщення користувача (нагадування про перегляд) | авторизовані |
| POST | `/catalog/notifications/read` | Позначити сповіщення прочитаними | авторизовані |
| GET | `/catalog/my-content` | Матеріали, за які відповідає користувач | авторизовані |
| POST | `/catalog/my-content/email-digest` | Увімкнути / вимкнути підсумок на пошту | авторизовані |
| POST | `/catalog/resources/{id}/confirm-review` | Підтвердити актуальність матеріалу | власник або менеджер |
| GET | `/auth/config` | Доступні способи входу | без авторизації |
| GET | `/auth/entra/start` | Початок корпоративного входу (OIDC + PKCE) | без авторизації |
| GET | `/auth/entra/callback` | Повернення з каталогу Entra ID | без авторизації |
| GET | `/auth/entra/logout` | Посилання виходу з корпоративної сесії | авторизовані |
