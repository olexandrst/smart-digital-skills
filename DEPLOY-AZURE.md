...
| `AUTO_MIGRATE` | `0` (типово) | Схему накатує Alembic; `1` повертає стару автоміграцію при старті |
...
## Крок 5. Застосувати міграції та створити початкові дані

Схема ведеться **версійними міграціями** (Alembic) — вона більше не
доповнюється сама при старті. Це свідома зміна: на кілька процесів і на
PostgreSQL автоматична правка схеми під час запуску небезпечна, бо кожен
воркер намагається змінити базу одночасно.
...
`init_db` сам розрізняє два випадки:

* **чиста база** — застосовує міграції (`alembic upgrade head`);
* **база, створена до переходу на міграції** — доганяє відсутні колонки старим
  способом і позначає схему поточною версією, щоб Alembic не намагався
  створити наявні таблиці повторно.

Замість `init_db` можна викликати Alembic напряму: `alembic upgrade head`.

Обидва скрипти **ідемпотентні** — повторний запуск нічого не зіпсує.
...
3. Застосуйте міграції — **це окремий крок, застосунок більше не робить його сам**:

```bash
az webapp ssh --name $APP --resource-group $RG
cd /home/site/wwwroot
cp /home/data/profihub.db /home/data/profihub-$(date +%F).db   # копія перед змінами
alembic upgrade head
```

`/home/data` деплой не перезаписує, тож контент і користувачі зберігаються.
Довідники (види матеріалів, статуси, рівні зрілості, словник формулювань)
синхронізуються при старті автоматично — це дані, а не схема.

> **Запасний варіант.** `AUTO_MIGRATE=1` повертає стару поведінку: застосунок
> сам дописує відсутні колонки при старті. Придатне для однопроцесних
> інсталяцій на SQLite; на PostgreSQL і при кількох воркерах користуйтеся
> міграціями.
...
Корпоративний вхід вмикається змінними середовища — коду міняти не треба.
Поки `ENTRA_TENANT_ID` або `ENTRA_CLIENT_ID` порожні, застосунок працює на
парольному вході, як і раніше.
...
3. У розділі **Token configuration** додайте claim **groups** (або **roles**,
   якщо використовуєте App Roles) — без нього ролі за групами не працюватимуть.
4. У **Certificates & secrets** створіть client secret, якщо не
   використовуєте PKCE без секрету (застосунок підтримує обидва варіанти).
5. Скопіюйте **Directory (tenant) ID** та **Application (client) ID**.
...
єдине джерело правди, тому видалення зі групи знімає доступ при наступному
вході. Якщо співробітник раніше заходив за паролем, обліковий запис
прив'язується за поштою, а не дублюється.

Вихід через `/api/auth/entra/logout` завершує й корпоративну сесію, а не лише
локальну.

> **Перед вимкненням парольного входу** (`ENTRA_ALLOW_PASSWORD_LOGIN=0`)
> переконайтеся, що хоча б один адміністратор входить через каталог —
> інакше в застосунок не зайде ніхто.
